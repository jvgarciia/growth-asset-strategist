...
4. **After any major change, update `PROJECT_CONTEXT.md`.**
   If you change the pipeline, add a step, modify a schema, alter a token budget, add an asset type, reposition the product, or make any architectural decision — record it before ending the session. The next session's Claude has no memory of this one.
...
| API proxy | `api/claude.js` (Vercel serverless function) | CORS + API key protection |
| Local proxy | `local-server.js` (Node.js HTTP server) | Used only for local dev without Vercel |
| API | Anthropic `claude-sonnet-4-6` via `/v1/messages` | Haiku is unavailable on this account |
...
The frontend cannot call the Anthropic API directly (CORS + key exposure). In production: `api/claude.js` (Vercel serverless). Locally: `local-server.js` on port 3000.
...
**Always use `extractJSON()` on all Claude API responses.** Never `JSON.parse()` raw output — Claude sometimes wraps JSON in markdown fences.
...
**`PROJECT_CONTEXT.md` is the source of truth.** This file (`CLAUDE.md`) is the operating protocol. They serve different purposes:

| File | Purpose |
|------|---------|
| `CLAUDE.md` | Session startup protocol. How Claude should think and behave. |
...
- `api/claude.js` is the Vercel serverless function at `/api/claude`
...
3. Syntax error in `api/claude.js` (node function crash = "Serverless Function crashed")
...
When working in this project, Claude should behave like a senior SaaS product engineer who also thinks like a marketing strategist.
...
- [ ] Did I use `extractJSON()` (not raw `JSON.parse()`) for any new Claude API response handling?
...
- [ ] Does the fetch URL in `index.html` use `/api/claude` (not `localhost:3000`)?
...
Claude Code can use the Chrome extension / browser control tools to inspect and test the live app directly. Use this capability to verify UI changes and catch issues before reporting them.
...
## 16. Codex Review Workflow

Codex is a secondary reviewer and debugging agent. Claude Code remains the primary implementation agent and product architect. Codex does not make architectural decisions and does not edit files concurrently with Claude Code unless on a separate branch or worktree.

**When to invoke Codex:**
- After implementing a large feature
- When a bug is hard to isolate and a second set of eyes is needed
- When a deployment fails and the root cause is unclear
- Before pushing a change that is risky or touches many functions
- When adversarial review of a completed implementation would be valuable

**What Codex should check:**
- Syntax issues and broken function signatures
- Duplicate logic or conflicting declarations
- Fragile selectors, field accesses, or assumptions about response shape
- Export, build, or routing problems
- Obvious UX regressions introduced by the change
- Security risks — hardcoded secrets, exposed keys, unvalidated inputs

**Deployment gate for major changes:**
1. Claude Code implements the change
2. Browser testing validates the affected flows (Section 15)
3. Codex reviews if the change is large, touches the pipeline, or is going to production
4. Then deploy

**Constraint:** Do not let Codex and Claude Code edit the same files simultaneously unless working on separate branches or worktrees. Codex output is advisory — Claude Code evaluates it before acting on any suggestion.

---

*This file is auto-loaded by Claude Code at session start. It is the operating protocol, not documentation. Update it when the workflow changes, not when the product changes — product changes go in `PROJECT_CONTEXT.md`.*
